...
bool StringHasPrefix(const std::string& sString, const std::string& sPrefix) {
	if (sPrefix.size() > sString.size()) return false;

	// Use std::equal to compare the beginning of sString with sPrefix
	return std::equal(sPrefix.begin(), sPrefix.end(), sString.begin(),
		[](char a, char b) {
